    ...
    <ul class="search-list-wrapper w-100 pl-0">
        <div class="keyword mb-2">
            @if (sizeof($keywords) > 0)
                <div class="title m-0">{{__('Popular Suggestions')}}</div>
        ...
        <div class="category mb-2">
            @if (count($subsubcategories) > 0)
                <div class="title m-0">{{__('Category Suggestions')}}</div>
                <ul style="list-style: none">
        ...
        <div class="product">
            @if (count($products) > 0)
                <div class="title m-0">{{__('Products')}}</div>
                <ul style="list-style: none">
                    @foreach ($products as $key => $product)
                        <li>
                            <a href="{{ route('product', $product->slug) }}">
                                <div class="d-flex search-product align-items-center">
                                    <div class="image" style="background-image:url('{{ file_exists(($product->thumbnail_img)) ? asset($product->thumbnail_img) : asset('frontend/images/placeholder.jpg') }}');">
                                    </div>
